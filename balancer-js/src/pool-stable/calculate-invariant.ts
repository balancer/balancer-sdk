...
export const calculateStableInvariant = (
  ...
  const ampTimesTotal = BigInt(amplificationParameter) * BigInt(numTokens);
  ...
  for (let i = 0; i < 256; i++) {
    ...
    normalizedBalances.forEach((normalizedBalance) => {
      ...
      D_P = SolidityMaths.divDownFixed(
        ...
        SolidityMaths.mul(BigInt(normalizedBalance), BigInt(numTokens))
      );
    });
