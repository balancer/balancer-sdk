...
(async function () {
  ...
  console.log(
    `${poolQuery.pool?.tokens
      ?.map((token) => `${token.symbol}\t${token.weight}`)
      .join('\n')}`
  );
  let result = await client.JoinExits({ where: { pool: POOL_ID } });

  const userId = result.joinExits.sort((a, b) => a.timestamp - b.timestamp)[0]
    .user.id;
  console.log(`${userId}`);
  ...
  result.joinExits
    ...
    .forEach((item) =>
      console.log(
        `${item.id}\t${new Date(item.timestamp * 1000).toLocaleString()}\t${
          item.type
        }\t${item.amounts}`
      )
    );
