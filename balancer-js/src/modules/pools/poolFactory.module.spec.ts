...
const SEED_TOKENS: Array<SeedToken> = [ 
    ...
    { id: 0, tokenAddress: ADDRESSES[42].USDC.address, weight: 40, isLocked: false, amount: "200000000" },
    { id: 0, tokenAddress: ADDRESSES[42].WBTC.address, weight: 30, isLocked: false, amount: "200000000" } 
];
...
describe('pool factory module', () => {
    context('WeightedPool', () => {
        ...
        it('should create a weighted pool', async () => {
            ...
            expect(creationTxParams.name).to.eq('30DAI-40USDC-30WBTC')
    ...
    context('handleCreation', async () => {
        ...
        it('should return the pool ID from the issuing transaction', async () => {
            ...
            const { id } = await balancer.pools.getPoolInfoFromCreateTx(tx);
