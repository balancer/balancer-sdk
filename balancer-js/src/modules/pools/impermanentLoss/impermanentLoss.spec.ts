...
import {MockPriceProvider} from "@/test/lib/ImpermanentLossData";
...
const service = new ImpermanentLossService(
  new MockPriceProvider(new CoingeckoPriceRepository([], 1), new AaveRates('', sdk.rpcProvider, Network.MAINNET)),
);
...
describe('ImpermanentLossService', () => {
  context('service.getWeights', () => {
    ...
    it('should throw error if missing weight', async () => {
      ...
      try {
        ...
        expect(e.message).eq(BalancerError.getMessage(BalancerErrorCode.MISSING_WEIGHT))
      }
    });
  });
  context('service.getDelta', () => {
    ...
    it('should throw an error for wrong parameter', async () => {
      try {
        ...
        expect(e.message).eq(BalancerError.getMessage(BalancerErrorCode.ILLEGAL_PARAMETER))
      }
    });
  });
  context('service.getEntryPrices', () => {
    it('should return prices for tokens', async () => {
      ...
      expect(prices["0x0d500b1d8e8ef31e21c99d1db9a6444d3adf1270"]).eq(0.9993785272283172);
      expect(prices["0x3a58a54c066fdc0f2d55fc9c89f0415c92ebf3c4"]).eq(1.9996776052990013);
    });
  ...
  context('service.getAssets', () => {
    it('should returns a list of assets with deltas and weights', async () => {
      ...
      const entryPrices = { "0x0d500b1d8e8ef31e21c99d1db9a6444d3adf1270": 10, "0x3a58a54c066fdc0f2d55fc9c89f0415c92ebf3c4": 15 };
      const exitPrices  = { "0x0d500b1d8e8ef31e21c99d1db9a6444d3adf1270": 15, "0x3a58a54c066fdc0f2d55fc9c89f0415c92ebf3c4": 10 };
      const assets = service.getAssets(poolTokens, exitPrices, entryPrices, weights);
  ...
  context('service.calculateImpermanentLoss', () => {
    context('for uniform distributed tokens', () => {
      it('should return proper value for deltas [0.2%, -49.89%]', async () => {
        ...
        const IL = service.calculateImpermanentLoss(poolValueDelta, holdValueDelta);
        expect(IL).eq(5.72);
      });
