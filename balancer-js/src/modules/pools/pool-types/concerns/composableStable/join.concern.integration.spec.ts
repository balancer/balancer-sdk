  ...
  describe('Composable V1', async () => {
    ...
    context('join transaction - join with params', () => {
      ...
      before(async function () {
        ...
        const sendJoinTransaction =
          sdk.contracts.vault.connect(signer).joinPool;
  ...
  describe('Composable V2', async () => {
    ...
    before(async function () {
      ...
      const poolObj = pools_16350000.find(
        ({ id }) =>
          id ==
          '0xa13a9247ea42d743238089903570127dda72fe4500000000000000000000035e'
      ) as unknown as Pool;
      // const initialBalances = poolObj.tokens.map((token) =>
      //   parseFixed('100000', token.decimals).toString()
      // );
    ...
    context('join', () => {
      it('should fail', () => {
        ...
        const slippage = '0';
        pool.buildJoin(signerAddress, tokensWithoutBpt, amountsIn, slippage);
      });
    });
    // context('join transaction - join with encoded data', () => {
    //   const amountsIn = [
