...
describe('sortTokens', () => {
    ...
    context('when provided only tokens', () => {
        context('when provided only ERC20s', () => {
            it('sorts the tokens in ascending order', () => {
                ...
                const [sortedTokensWithWeth] = assetHelpers.sortTokens(
                    UNSORTED_TOKENS_WITH_WETH
                );
                expect(sortedTokensWithWeth).to.be.deep.eq(
                    SORTED_TOKENS_WITH_WETH
                );
            });
        });
    ...
    context('when provided additional arrays', () => {
        ...
        it('maintains relative ordering with tokens array', () => {
            const [sortedTokens, sortedNumbers, sortedLetters] =
            ...
            sortedTokens.forEach((token, index) => {
                ...
                expect(sortedNumbers[index]).to.be.eq(
                    UNSORTED_NUMBERS[unsortedIndex]
                );
                expect(sortedLetters[index]).to.be.eq(
                    UNSORTED_LETTERS[unsortedIndex]
                );
            });
        });
    });
