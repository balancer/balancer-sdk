...
async function queryBatchSwapWithSor(sor, vaultContract, queryWithSor) {
    ...
    const returnAmounts = Array(returnTokens.length).fill(Zero);
    let deltas = Array(batchedSwaps.assets.length).fill(Zero);
    try {
        // Onchain query
...
const SUBGRAPH_URLS = {
    ...
    [Network.GÖRLI]: 'https://api.thegraph.com/subgraphs/name/balancer-labs/balancer-goerli-v2',
    [Network.KOVAN]: 'https://api.thegraph.com/subgraphs/name/balancer-labs/balancer-kovan-v2',
    [Network.POLYGON]: 'https://api.thegraph.com/subgraphs/name/balancer-labs/balancer-polygon-v2',
...
export { AaveHelpers, AssetHelpers, BalancerErrors, BalancerSDK, ManagedPoolEncoder, Network, PoolBalanceOpKind, PoolSpecialization, RelayerAction, RelayerAuthorization, RelayerService, SUBGRAPH_URLS, StablePhantomPoolJoinKind, StablePoolEncoder, StablePoolExitKind, StablePoolJoinKind, SwapType, SwapsService, UserBalanceOpKind, WeightedPoolEncoder, WeightedPoolExitKind, WeightedPoolJoinKind, accountToAddress, getLimitsForSlippage, getPoolAddress, getPoolNonce, getPoolSpecialization, isNormalizedWeights, isSameAddress, signPermit, splitPoolId, toNormalizedWeights };
